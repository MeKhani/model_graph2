...
def extract_type_groups_for_nell(
    ...
    """Extract type → [ent_idx] mapping for NELL (different separator)."""
    if type2id is None:
        type2id = {}
...
def triples_from_primekg_df(df: pd.DataFrame) -> List[IndexedTriple]:
    """Convert PrimeKG DataFrame to indexed triples."""
...
def _process_inference_split(
    ...
    if ent_map is not None and type2id is not None:
        type_groups_ind, ind_ent_map, type2id = extract_type_groups(
...
def data_to_pickle(args: Any) -> None:
    ...
    if args.benchmark == "dataset/new_data":
        ...
        if args.model_graph_type == "relation_base":
        ...
        elif args.model_graph_type == "entity_base" and "hetionet" in data_name.lower():
            train_idx, rel_map, ent_map = create_full_index(train_raw)
            type_groups_tr, ent_map, type2id = extract_type_groups(train_raw, ent_map)
            valid_idx = remap_to_known_ids(valid_raw, ent_map, rel_map)
            
            ind_result = _process_inference_split(
                data_dir, args.test_type, rel_map, ent_map, type2id
            )
            ...
            print(f"Ind  entity types count: {len(ind_result.get('ent_type', {}))}")

        else:
            raise ValueError(f"Unknown configuration for dataset {data_name}")

    else:
        # ── Standard benchmark ( inductive) ───────────
